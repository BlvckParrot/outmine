...
/** The analytics origin, or nothing. One origin in two directives - the tag is fetched
 *  from it and the beacons are sent back to it - and both are empty unless the pair in
 *  config.analytics is complete, so an unconfigured deployment keeps today's policy to
 *  the character. */
const analyticsSrc = config.analytics.origin && config.analytics.siteId
  ? [config.analytics.origin]
  : [];

/** Where the browser may post errors. connect-src only: unlike the analytics tag, this
 *  SDK is bundled with the app rather than fetched, so script-src is untouched. */
const observeSrc = observeConfigured() ? [config.observe.origin] : [];

app.use(secureHeaders({
  contentSecurityPolicy: {
    defaultSrc: ["'self'"],
    scriptSrc: [NONCE, "'self'", "'wasm-unsafe-eval'", ...analyticsSrc],
    styleSrc: ["'self'", "'unsafe-inline'"],
    imgSrc: ["'self'", "data:"],
    fontSrc: ["'self'"],
    connectSrc: ["'self'", ...analyticsSrc, ...observeSrc],
    workerSrc: ["'self'", "blob:"],
    objectSrc: ["'none'"],
    baseUri: ["'self'"],
    frameAncestors: ["'none'"],
